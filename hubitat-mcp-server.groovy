...
- `addTriggers` / `addActions` (bulk path): `updateRuleFailed: true` + `subscriptionsNotLive: true` + `updateRuleError: <message>` with the same `success`/`partial` flip. The per-item adds IS committed (triggers/actions arrays still surface on the success-shape keys) but the running rule instance never re-subscribed -- retry as above. Bulk items fail closed: the first item that returns `success:false` or `partial:true` stops the request, later items (and, on create, a later Required Expression) return `notAttempted:true`, no trailing `updateRule` or Done fires, and the result carries `bulkStoppedAfter` + `finalisationNotAttempted:true` (the same applies to `replaceActions`).
