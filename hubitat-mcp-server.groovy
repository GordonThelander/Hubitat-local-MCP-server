...
- Bulk early stop (`addTriggers` / `addActions`, create, and `replaceActions`): the first item that returns `success:false` or `partial:true` stops the request. Later items (and, on create, a later Required Expression) return `notAttempted:true`, no trailing `updateRule` or Done fires, and the result carries `bulkStoppedAfter` + `finalisationNotAttempted:true` instead of `updateRuleFailed`. Repair the failed item and add the not-attempted items, or restore the pre-op backup, then fire `updateRule` once the rule is complete.
